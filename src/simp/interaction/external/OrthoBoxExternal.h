#ifndef SIMP_ORTHO_BOX_EXTERNAL_H
#define SIMP_ORTHO_BOX_EXTERNAL_H
...
namespace Simp
{
   ...
   /**
   * A repulsive 9-3 potential confining particles inside an orthogonal box
   * along directions specified by indConfine_.
   *    indConfine_ = 0: x & y & z directions
   *                = 1: y & z directions
   *                = 2: z directions
   *                = 3: no confinement
   *
   ...
   */
   class OrthoBoxExternal : public ParamComposite 
   {
   ...
   public:
      ...
      * Copy constructor.
      */
      OrthoBoxExternal(const OrthoBoxExternal& other);
   ...
   private:
      ...
      /// Index to the direction of confinement.
      int indConfine_;
   ...
   */
   inline double OrthoBoxExternal::energy(double d, int type) const
   {
      if (d < cutoff_) {
         double r3i;
         r3i = d * d * d;
   ...
   */
   inline double OrthoBoxExternal::energy(const Vector& position, int type) const
   {
      double d, totalEnergy;
      Vector halfL = boundaryPtr_->lengths();
      halfL *= 0.5;
      totalEnergy = 0.0;
      for (int i = indConfine_; i < Dimension; ++i) {
         d = position[i];
   ...
   */
   inline double OrthoBoxExternal::forceScalar(double d, int i) const
   {
      if (d < cutoff_) {
         double r3i;
         r3i = d * d * d;
   ...
   */
   inline void 
   OrthoBoxExternal::getForce(const Vector& position, int type, Vector& force) const
   {
      double d = position[2];
      double scalarf;
      Vector halfL = boundaryPtr_->lengths();
