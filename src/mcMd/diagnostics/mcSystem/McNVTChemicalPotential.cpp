...
namespace McMd
{
   ...
   */
   void McNVTChemicalPotential::sample(long iStep) 
   {
      if (isAtInterval(iStep))  {
         ...
         #endif
         double beta, length;
         // dummy variables to keep the rosenbluth factor and energy of inserted bonds in each stage.
         double w = 0, de = 0;
         ...
         for (int i = 0; i < nMoleculeTrial_; i++) {
            ...
            length = system().bondPotential().randomBondLength(&random(), beta, bondTypeId);
            ...
            angle = system().anglePotential().randomAngle(&random(), beta, angleTypeId);
   ...
   */
   void
   McNVTChemicalPotential::addEndAtom(Molecule* molPtr, int atomId, double &rosenbluth, double &energy)
   {
      ...
      beta   = energyEnsemble().beta();
      length = system().bondPotential().randomBondLength(&random(), beta, molPtr->bond(atomId-1).typeId());
      #ifdef INTER_ANGLE
      Vector n = pvt1Pos;
      n -= pvt2Pos;
      angle = system().anglePotential().randomAngle(&random(), beta, molPtr->angle(atomId-2).typeId());
      #endif
      #ifdef INTER_DIHEDRAL
      Vector dR1 = pvt3Pos;
      Vector dR2 = pvt2Pos;
      ...
      for (iTrial=0; iTrial < nTrial_; ++iTrial) {
         ...
         if (system().hasDihedralPotential()) {   
            trialEnergy[iTrial] += system().dihedralPotential().energy(dR1, dR2, dR3, molPtr->dihedral(atomId-3).typeId());
         }
         #endif
   ...
   */
   void McNVTChemicalPotential::uniformCone(const double length, const double angle, const Vector n, Vector &p)
   {
      Vector v1;
