...
namespace McMd
{
   ...
   * generalized system.
   */
   void McMdConfigIo::transformCartConfigToGen(std::istream &in)
   {
      ...
      for (iSpecies = 0; iSpecies < simulation().nSpecies(); ++iSpecies) {
         ...
         for (iMol = 0; iMol < nMolecule; ++iMol) {
            ...
            // Read positions, shift into primary cell
            for (molPtr->begin(atomIter); atomIter.notEnd(); ++atomIter) {
               transformCartToGen(in, *atomIter);
            }
   ...
   * to file.
   */
   void McMdConfigIo::transformGenToCartConfig(std::ostream &out)
   {
      using std::endl;
      ...
      for (iSpecies = 0; iSpecies < simulation().nSpecies(); ++iSpecies) {
         ...
         for ( ; molIter.notEnd(); ++molIter) {
            ...
            for (molIter->begin(atomIter); atomIter.notEnd(); ++atomIter) {
               //out << atomIter->position() << endl;
               transformGenToCart(out, *atomIter);
            }
            ++iMolecule;
         }
      }
